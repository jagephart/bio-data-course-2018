...
You will want your data to be factors when you are plotting lines, colors, etc. by that variable or faceting on that variable. The facor order defaults to alphabetical order or numeric order (by first digit), but we often want to specify a different order that makes more sense for communicating our results (especially if the data has a natural order, like small, medium and large). 
